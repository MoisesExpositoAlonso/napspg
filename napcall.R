...
require(argparser)
require(Rcpp)
sourceCpp("napspg/src/NAP.cpp")
source("napspg/R/napML.R")
...
# Analysis conditions
p <- add_argument(p, "--l", help="number of SNPs to analyze", default=1000)
...
p <- add_argument(p, "--k", help="cross-validations of the dataset, k=10 default", default=5)
# Output behaviour
...
finalinfo<-paste0(finalbase,".info")
...
write.table(quote=F,row.names=F,
            file=finalinfo,
            x=data.frame(
              mod=argv$m,
              epi=argv$e,
...
d<-data.frame(phenotype=bname,
            ...
            mod=mod,
            epi=epi,
            r=c(res$rspearman,res$rnonzero),
            r_in=c(res$rspearman_in,res$rnonzero_in)
            )
write.table(row.names = F, quote = F, 
            x=d,
            file=finaltsv
            )
